...
function [p] = aAAC_runExperiment_MAST_noJoystickAlternative(subject, runNumber,comment)              % args can be omitted and entered via GUI prompt
...
while runNumber <= totalRuns
    ...
    if ~exist('comment', 'var')
        comment = 'joystick could not be used - button box used instead';
    end
    if runNumber < 2
    ...
    function Trial(itd, itd2,itd3, punishLevel, rewardLevel, topAversive, shockStrengthInmA)
        ...
        while GetSecs() < choicePhaseEndTime
            ...
            while (GetSecs() < (vbl + p_ptb_ifi/2.5)) && (lock == 0)
                ...
                yTrace(yTraceCount) = joyCalYPosition - newY;
                [keyIsDown, ~, keyCode] = KbCheck(-1);
                if keyIsDown
                    if find(keyCode) == p.keys.up
                        newY = newY - 1;
                    elseif find(keyCode) == p.keys.down
                        newY = newY +1;
                    end
                end
                ...
                if (joyInitMove == 0) && (yTrace(yTraceCount)~=joyCalYPosition)
